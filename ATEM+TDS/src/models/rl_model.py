...
class Model(nn.Module):
    ...
    def _base_summary_generate(self, batch, topic_info):
        ...
        sent_hid = self.sent_encoder(src_emb, mask_src==0)[:, 0, :]
        ...
        # print('sent_input.size()',sent_input.size())
    ...
    def _pointer_net_decoding(self, batch, memory_bank, init_tokens, max_length,
        ...
        for step in range(max_length):
            ...
            # Avoid duplicate extraction
            # print("shape(dump_mask): ",dup_mask)
            # print("shape(F.one_hot(ids, dist_size).byte()):", F.one_hot(ids, dist_size).byte())
            dup_mask += (F.one_hot(ids, dist_size).byte()==1)
    ...
    def _fast_translate_batch(self, batch, memory_bank, max_length, init_tokens=None, memory_mask=None,
        ...
        for step in range(max_length):
            ...
            topk_beam_index = topk_ids.div(vocab_size, rounding_mode="floor")
            topk_ids = topk_ids.fmod(vocab_size)
            # print("topk_ids:", topk_ids)

            # Map beam_index to batch_index in the flat representation.
            # print("topk_beam_index:", topk_beam_index)
            # print("topk_beam_index.size(0):", topk_beam_index.size(0))
            batch_index = (
                    topk_beam_index
    ...
    def _topic_vec_ge(self, topic_info, agent_mask, cust_mask, idx, vec):
        ...
        if self.args.split_noise:
            ...
            fused_vec_summ = (torch.tensor(1)-gate_summ) * topic_vec[0] + gate_summ * self.topic_emb_linear_summ(mapped_vec)

            gate_noise = torch.sigmoid(self.topic_gate_linear_noise(torch.cat([mapped_vec, topic_vec[1]], dim=-1)))
            fused_vec_noise = (torch.tensor(1)-gate_noise) * topic_vec[1] + gate_noise * self.topic_emb_linear_noise(mapped_vec)

            fused_vec = (fused_vec_summ, fused_vec_noise)
        else:
            gate = torch.sigmoid(self.topic_gate_linear(torch.cat([mapped_vec, topic_vec], dim=-1)))
            fused_vec = (torch.tensor(1)-gate) * topic_vec + gate * self.topic_emb_linear(mapped_vec)
        return fused_vec
    ...
    def pretrain(self, batch):
        ...
        if self.training:
            ...
            decode_output, _, _ = self.decoder(tgt[:, :-1], top_vec, dec_state,
                                               memory_masks= (mask_src_mapped == 0),
                                               topic_vec=topic_vec_ge)
            summary = None
        else:
            decode_output = None
    ...
    def forward(self, batch):
        ...
        if self.args.encoder == "bert":
            ...
            src_mapped_emb = self.embeddings(src_mapped)
            top_vec = self.encoder(src_mapped_emb, mask_src_mapped==0)
